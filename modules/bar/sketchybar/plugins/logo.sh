...
# click gestures. Defined in sketchybarrc, configured by haus.bar.logo.*.
#
...
#   update  a newer haus is pinned upstream (haus.bar.logo.updateCheck)
#   ok      neither
#
...
# where it was, and a dead `bar-bottom` leaves its bar on screen drawing the
...
health() {
    ...
    for pair in "org.nixos.aerospace:AeroSpace" \
        "org.nixos.bar-bottom:bar-bottom" \
...
upstream_behind() {
    [ "$BAR_LOGO_UPDATE_CHECK" = "1" ] || return 1
    ...
    [ -f "$lock" ] || return 1
    rev="$(jq -r '.nodes.haus.locked.rev // ""' "$lock" 2>/dev/null)"
    [ -n "$rev" ] || return 1
    owner="$(jq -r '.nodes.haus.original.owner // "hausfold"' "$lock" 2>/dev/null)"
    repo="$(jq -r '.nodes.haus.original.repo // "haus"' "$lock" 2>/dev/null)"
    ref="$(jq -r '.nodes.haus.original.ref // "HEAD"' "$lock" 2>/dev/null)"
...
rebuild() {
    ...
    [ -x "$BAR_LOGO_COMMANDS/rebuild.sh" ] || return 0
