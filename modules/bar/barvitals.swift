// barvitals — one sample of what this Mac is busy with, for the cpu and memory
...
guard arguments.first == "sample" else {
  FileHandle.standardError.write(
    Data("usage: barvitals sample --state PATH [--top cpu|mem|none] [--rows N]\n".utf8))
  exit(2)
}
...
guard !statePath.isEmpty else {
  FileHandle.standardError.write(Data("barvitals: --state is required\n".utf8))
  exit(2)
}
