...
let
  ...
  # instructions/skill files and the per-client hook and settings wiring —
  # hosted here on the theory that a home profile is where they can be written.
  # That turned out to be a habit rather than a constraint: modules/ai writes
  # into the same `home-manager.users.<name>` and home-manager merges the two,
  # and every per-client fact is one record in modules/ai/clients now. What is
  # left here is the terminal's own business: the dotfiles this room themes,
  # opencode's included.
...
in
{
  ...
  home-manager.users.${username} =
    ...
    in
    {
      ...
      # merges the two attrsets, and so are each client's hook files
      # (modules/ai/clients). What is left below is dotfiles: theme drops, and
      # scruff's config plus the lane scripts while the AI room is on.
      home.file = {
