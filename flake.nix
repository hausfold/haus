      ...
      checks = nixpkgs.lib.genAttrs allSystems (
        ...
        let
          ...
          scaleAt =
            ...
            in
            {
              ...
              numbers = {
                ...
                # not the bar's own: it IS windows's outer side gap on the
                # narrower display (modules/lib/gaps.nix), so the outermost pill
                # lines up with the tiled window below it.
          ...
          expectedScaleTable = ''
            ...
            file .config/sketchybar/bar.sh moves
          ...
          # The activation text `haus.appStore.install` actually emits, for
          # app-store-activation below. `unsafeDiscardStringContext` because the
          # check reads the SHAPE of the mas calls and nothing else: a
          # context-carrying string would make the builder realise core's whole
          # closure just to look at it.
          appStoreActivation =
            builtins.unsafeDiscardStringContext
              (standaloneSystem [
        ...
        in
        {
          ...
          bar-plugins-executable = pkgs.runCommand "haus-bar-plugins-executable-ok" { } ''
            libs="aerospace_lib.sh ai-provider.sh front_app_lib.sh media_lib.sh vitals_lib.sh workspace_lib.sh"
            bad=
