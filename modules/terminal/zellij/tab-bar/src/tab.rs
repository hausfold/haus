...
// here" was worth. Count lives only in bar's popup now; the tab-bar just
...
pub fn render_tab(
    ...
    // The same three colours bar's `agents` pill uses — peach "needs you", sky
    ...
    // these resolve to exactly bar's peach/sky/green.
    ...
    // bar's popup's job, not a tab cell's.
