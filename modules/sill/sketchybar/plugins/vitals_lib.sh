...
# ── the sample ────────────────────────────────────────────────────────────────
# Sets the globals below from one `sillvitals` run. Anything the sampler
...
# sillpop is: this runs from launchd's PATH, where nothing nix-shaped is on it.
...
VITALS_BIN="${VITALS_BIN:-/run/current-system/sw/bin/sillvitals}"
...
# sillpop so a click ANYWHERE closes it (sketchybar alone only hears clicks on
