...
| Every other tool's skill | `~/.claude/skills/<name>/` | Same option, same fan-out: each hausfold tool names and ships its own (scruff's `scruff` and `handoff`, factory's `factory` and `nightshift`, plus trill's `trill` when `haus.notifications.compositor` is on). Store symlinks — edit them in the tool's repo, not here. `readlink` points at haus's own checked copy rather than at the tool's derivation, so it names the skill and not the tool it came from. |
