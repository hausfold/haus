...
# Apple killed in macOS 15.4 (see modules/bar/media-control.nix).
#
...
build_popup() {
    ...
    if [ -n "$art" ] && [ -s "$art" ]; then
        ...
        ARGS+=(--add item media.popup.art popup.media
            ...
            width="$BAR_MEDIA_ART_BOX" background.height="$BAR_MEDIA_ART_BOX"
    ...
    if [ -n "$label_app" ]; then
        ...
        if { [ -z "$art" ] || [ ! -s "$art" ]; } &&
            ...
            ARGS+=(--add item media.popup.appicon popup.media
                ...
                icon.drawing=off icon.padding_left=0 icon.padding_right=0
                label.drawing=off label.padding_left=0 label.padding_right=0
                background.height="$BAR_MEDIA_BADGE_BOX"
                background.drawing=on background.color=0x00000000
                background.image="app.$label_app"
                background.image.scale="$BAR_MEDIA_BADGE_SCALE"
                background.image.corner_radius=6
                background.image.drawing=on
...
open_popup() {
    ...
    # above. SKETCHYBAR_BIN is what tells barpop WHICH bar to guard: unset, it
    # queries the top one, finds no such item on a pill that moved to the bottom
    # bar, and exits before it ever arms.
    SKETCHYBAR_BIN="$SB" /run/current-system/sw/bin/barpop arm media 2>/dev/null &
}
...
scroll_seek() {
    ...
    mkdir "$BAR_MEDIA_STATE_DIR/scroll.lock" 2>/dev/null || return
    (
        sleep 0.4
