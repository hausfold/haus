      ...
      checks = nixpkgs.lib.genAttrs allSystems (
        ...
        let
          ...
          expectedDesktopDiagnostics = ''
            ...
            test/desktops/host-only-command.nix: haus.keys.leaderExtras.*.command is host-only, so a shared desktop may not set it. It is a shell command this machine runs. A desktop is a file you can read to know what it does, and a leaf that runs a command is exactly what would stop being true of.
            ...
            test/desktops/host-only-identity.nix: haus.git.email is host-only, so a shared desktop may not set it. It names you rather than a machine — your commit identity, the addresses that are yours. A desktop that set it would put its author's name on your work.
            ...
            test/desktops/host-only-secret.nix: haus.focus.slack.tokenCommand is host-only, so a shared desktop may not set it. It points at a secret, or at the store this machine keeps its secrets in — a credential belonging to one person on one Mac.
            test/desktops/host-only-secret.nix: haus.secrets.provider is host-only, so a shared desktop may not set it. It points at a secret, or at the store this machine keeps its secrets in — a credential belonging to one person on one Mac.
            ...
            test/desktops/host-only-widget-command.nix: haus.bar.widgets.<name>.command is host-only, so a shared desktop may not set it. It is a shell command this machine runs. A desktop is a file you can read to know what it does, and a leaf that runs a command is exactly what would stop being true of.
