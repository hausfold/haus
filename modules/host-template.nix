# The annotated host file — every `nebelhaus.*` option at its default, with its
...
pkgs.runCommand "nebelhaus-host-template-${version}"
  {
    ...
    meta = {
      description = "Annotated host file listing every nebelhaus.* option at its default";
      inherit version;
    };
  }
  ''
    ...
    grep -q '^  # nebelhaus\.' "$tmpl" \
      || { echo "host template rendered no nebelhaus.* options — the render is broken" >&2; exit 1; }
    ...
    # (`19, scaled by nebelhaus.ui.scale`) rendered as `… = 19, scaled by …;`,
    ...
    sed -E 's/^  # (nebelhaus\.)/  \1/' "$tmpl" > uncommented.nix
    ...
    want=$(jq -r '[to_entries[]
             ...
             ${optionsJSON}/share/doc/nixos/options.json)
    got=$(grep -c '^  nebelhaus\.' uncommented.nix || true)
    [ "$got" = "$want" ] \
