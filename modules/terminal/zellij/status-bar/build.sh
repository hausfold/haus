#!/bin/bash
# Builds the zellij-status-bar WASM plugin (nix cross-build, no rustup needed) and
# prints the .wasm path on stdout — the dev loop for feeling a change without a
# rebuild:
#
#   zscratch --plugin status-bar="$(./build.sh)"
#
...
echo "Building zellij-status-bar WASM plugin (pkgsCross.wasi32)..." >&2
...
echo "$out/bin/zellij-status-bar.wasm"
