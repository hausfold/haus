...
# from modules/terminal/default.nix's seedZellijPluginPermissions calls. A scratch
# plugin lives at a NEW path, so its grant isn't pre-seeded; we seed it here.
plugin_file() { # NAME -> the on-disk wasm basename terminal installs
  case "$1" in
...
# grants (different keys). Mirrors the awk dance in terminal's seed helper.
...
cmd_run() {
  ...
  if [ -n "$LAYOUT_SRC" ]; then
    ...
    # pinned to $HOME (mirrors terminal's replaceStrings). Keep it in step so a
