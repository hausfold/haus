#!/bin/bash
# Builds the zellij-link-handler WASM plugin (nix cross-build, no rustup needed) and
# prints the .wasm path on stdout — the dev loop for feeling a change without a
# rebuild:
#
#   zscratch --plugin link-handler="$(./build.sh)"
#
# A REBUILD DOES NOT RUN THIS. terminal builds ./default.nix itself (zellijPlugins
# in modules/terminal/default.nix), so the installed plugin always matches ./src
...
echo "Building zellij-link-handler WASM plugin (pkgsCross.wasi32)..." >&2
...
echo "$out/bin/zellij-link-handler.wasm"
