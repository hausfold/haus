      ...
      checks = nixpkgs.lib.genAttrs allSystems (
        ...
        let
          ...
          expectedDesktopDiagnostics = ''
            ...
            test/desktops/host-only-monitor.nix: haus.windows.workspaceMonitors.2 names a physical display, which is a fact about one desk — a desktop may only pin a workspace by position (`main`, `secondary`, `built-in`, or a number counting from the left)
          ...
          expectedDesktopShowSample = ''
            ...
            windows  haus.windows.workspaceMonitors.2 = "secondary"
          '';
