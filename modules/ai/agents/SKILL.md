...
- **TCC dialogs fire on first use and do not block.** The grant is written at
  the same instant the modal appears, so the command returns real data while an
...
Ask for the user's own screen only when the VM genuinely cannot answer the
question — their windows, their data, their hardware, or a guest that will not
boot. If the VM is out of reach at all, say so in one line and hand the
