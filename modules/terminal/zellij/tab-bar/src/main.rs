...
/// One agent pane's self-reported state, piped in by bar's agents-hook.sh from
...
/// across every agent pane it holds. Exact count lives in bar's popup, not
...
impl AgentState {
    /// The wire vocabulary is bar's, verbatim: the same working/waiting/idle
...
/// not a placeholder — bar's popup draws each row's client mark from the copy it
...
impl ZellijPlugin for State {
    fn load(&mut self, configuration: BTreeMap<String, String>) {
        ...
        // ask. terminal seeds both grants into zellij's permission cache, so this
        ...
        // it the agent paws simply never appear. It must be in terminal's seed list
    ...
    ///
    /// Note we never read bar's /tmp/haus-agents/*.state files: a plugin is
