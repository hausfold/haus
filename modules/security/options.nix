...
#
# security's options — the auth policy: who you prove you are to, and what you're
# excused from proving it for.
{ lib, ... }:

{
  options.haus = {
    security.touchId.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
    ...
    security.touchId.passwordlessRebuild = lib.mkOption {
      type = lib.types.bool;
      # NOT in-room taste, despite reading like it: this is an ungated root
      # grant (modules/security/default.nix writes the sudoers drop-in whatever
      # `security.touchId.enable` says), so the bare layer must not hand it out. A
