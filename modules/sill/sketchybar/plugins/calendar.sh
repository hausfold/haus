...
#
# Below `haus.sill.calendar.preciseUnder` hours the countdown carries minutes
...
# learn to ignore.
#
# For `haus.sill.calendar.imminent` minutes either side of the start the pill
...
#
# How wide the settled form is comes from `haus.sill.calendar.width`, applied as
...
# GENERATED from haus.sill.calendar.* — absent on a rice that predates it, hence
...
HORIZON=${SILL_CALENDAR_HORIZON:-24}       # hours ahead the pill counts down
PRECISE_UNDER=${SILL_CALENDAR_PRECISE_UNDER:-12}  # hours below which minutes show
IMMINENT=${SILL_CALENDAR_IMMINENT:-5}      # minutes either side of the start
PAST=${SILL_CALENDAR_PAST:-24}             # hours of "done" the dropdown keeps
UPCOMING=${SILL_CALENDAR_UPCOMING:-5}      # how many "next" rows the dropdown draws
ME_EXTRA=${SILL_CALENDAR_ME:-}             # comma-joined: addresses that are me
JOIN_EXTRA=${SILL_CALENDAR_JOIN_HOSTS:-}   # comma-joined: extra conferencing hosts
...
# items. SKETCHYBAR_BIN is how sillpop learns WHICH bar to guard: unset, it
