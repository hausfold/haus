  ...
  options.nebelhaus = {
    ...
    hearth.videoPlayer = {
      enable = lib.mkOption {
        ...
        default = false;
        description = ''
          Install IINA as the roster entry `iina` — a nixpkgs build, so it
          lands in ~/Applications/Home Manager Apps rather than /Applications.

          Off by default: which media player a machine wants is an editorial
          choice its owner makes, not something the shell room should decide
          by fetching a ~100 MB app bundle onto every host. Turn it on here,
          or install any player you like via the pounce "Install App" palette
          command or a roster entry of your own.

          Once on it is a roster entry like any other: give it a leader letter
          with `nebelhaus.roster.iina.key`, or pin a different build with
          `nebelhaus.roster.iina.package`.
        '';
      };
      ...
      claimFileTypes = lib.mkOption {
        ...
        description = ''
          ...
          QuickTime Player. Ignored unless the player is installed.
          ...
          and `hijackFileAssociations` claims it for the editor.

          This sets the USER default (via `duti`), the same thing Finder's
          Get Info ▸ Change All writes, so it is reversible by hand. Set false
          to install the app and leave every association alone.
        '';
      };
    };
