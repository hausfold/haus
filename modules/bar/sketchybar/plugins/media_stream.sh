...
# modules/bar/media-control.nix for the whole story and for why the replacement
...
LOCKDIR="/tmp/sketchybar_media_stream.$BAR_STREAM_KEY.lock"
...
# exactly one of them (a pill named in haus.bar.bottom.items MOVES rather than
...
rmdir "$BAR_MEDIA_STATE_DIR/art.lock" "$BAR_MEDIA_STATE_DIR/scroll.lock" \
    "$BAR_MEDIA_STATE_DIR/marquee.lock" 2>/dev/null
...
#   * the join is US (0x1f), not tab — see BAR_MEDIA_FS in media_lib.sh.
PREV_KEY=""
media_read_now && PREV_KEY="$(media_change_key)"

"$BAR_MEDIA_CONTROL" stream --no-diff --no-artwork --debounce=200 2>/dev/null |
    while IFS= read -r line; do
        ...
        # populated either way, and the widget's fetch hides the pill on it.
        ...
        if [ "$key" != "$PREV_KEY" ]; then
            ...
            if [ -n "$MEDIA_TITLE" ]; then
                ...
                rm -f "$BAR_MEDIA_ART".* "$BAR_MEDIA_ART_SCALE" "$BAR_MEDIA_TINT" 2>/dev/null
            fi
        fi
