...
let
  ...
  extra = {
    ...
    calendar = "Your next timed event, with a click-popup of the next five. A name too long for the pill is swept past rather than cut off with an ellipsis — it scrolls for a few seconds when the next event changes and then settles, and hovering brings it back; `haus.sill.calendar.width` sets how much room it gets. Pulls in `ical-buddy` automatically and reads Calendar, so macOS prompts for Calendar access on first run.";
  ...
  options.haus = {
    ...
    sill.calendar.width = lib.mkOption {
      ...
      description = ''
        ...
        not pixels. The label reads "<event> in <time>"; anything longer is
        clipped to this and swept past, the same way the media pill's title is.
