...
//     terminal pre-seeds zellij's permission cache (see modules/terminal).
...
//     leading `.`/`/` (e.g. `src/main.rs`, `modules/terminal/foo.rs`). Safe to
...
// whole. See modules/terminal/zellij/patches/naked-click-links.patch.
//
...
// e.g. `src/main.rs`, `modules/terminal/foo.rs`, `github.com/org/repo`. The last
...
/// (modules/terminal/zellij/patches/naked-click-links.patch); it is the whole
...
impl State {
    ...
    /// from haus.terminal.editor at build time, which is why the plugin (built
...
mod tests {
    ...
    fn parse_bare_url_rejects_non_web() {
        ...
        assert_eq!(parse_bare_url("modules/terminal/foo.rs"), None);
