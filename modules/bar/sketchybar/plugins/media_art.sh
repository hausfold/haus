...
LOCK="$BAR_MEDIA_STATE_DIR/art.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
    exit 0
fi
trap 'rmdir "$LOCK" 2>/dev/null' EXIT

PAYLOAD="$("$BAR_MEDIA_CONTROL" get 2>/dev/null)"
[ -n "$PAYLOAD" ] || exit 0
...
ART="$BAR_MEDIA_ART.${CURRENT_ID:0:8}.$EXT"
...
# than guessed once in the popup builder. Targeting BAR_MEDIA_ART_TARGET rather
# than the well's own width (BAR_MEDIA_ART_BOX) is what leaves the cover its
...
if [ -n "$ART_W" ] && [ "$ART_W" -gt 0 ] 2>/dev/null; then
    awk -v w="$ART_W" -v t="$BAR_MEDIA_ART_TARGET" 'BEGIN { printf "%.4f\n", t / w }' >"$BAR_MEDIA_ART_SCALE"
fi
...
BMP="$BAR_MEDIA_STATE_DIR/avg.bmp"
