#!/bin/bash
# Builds the zellij-tab-history WASM plugin (nix cross-build, no rustup needed) and
# prints the .wasm path on stdout — the dev loop for feeling a change without a
# rebuild:
#
#   zscratch --plugin tab-history="$(./build.sh)"
#
...
echo "Building zellij-tab-history WASM plugin (pkgsCross.wasi32)..." >&2
...
echo "$out/bin/zellij-tab-history.wasm"
