...
| Every other tool's skill | `~/.claude/skills/<name>/` | Same option, same fan-out: each hausfold tool names and ships its own (scruff's `scruff` and `handoff`, factory's `factory` and `nightshift`, nebelung's `nebelung`, plus the ones that follow a room — trill's `trill` with `haus.notifications.compositor`, pounce's `pounce` with `haus.launcher.enable`, perch's `perch` with `haus.shelf.enable`). Store symlinks — edit them in the tool's repo, not here. `readlink` points at haus's own checked copy rather than at the tool's derivation, so it names the skill and not the tool it came from. |
